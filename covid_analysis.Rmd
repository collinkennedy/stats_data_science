...
counties_vaccination_df %>% head(20)
...
#look at los angeles and sum daily deaths, then compare to actual number
counties_vaccination_df %>% drop_na(daily_deaths) %>% 
  filter(daily_deaths > - 1) %>% 
  filter(state == "California") %>% filter(county == "Los Angeles")

#count number of counties in california in this dataset and compare to actual number
counties_vaccination_df %>% filter(state == "California") %>% 
  group_by(county) %>% 
  summarize(total_counties = n_distinct(county))
...
ggplot(data = counties_vaccination_df, mapping = aes(x = date, y = total_deaths))+
  geom_line()

counties_vaccination_df

time_series = ts(data = counties_vaccination_df$daily_deaths, start = c())
time_series
...
```{r}
#make data
state_daily_deaths_df = counties_vaccination_df %>% drop_na(daily_deaths) %>% 
  filter(daily_deaths > -1) %>% 
  group_by(state) %>% summarize(daily_deaths = sum(daily_deaths)) %>% summarize(total_d = sum(daily_deaths))
state_daily_deaths_df

#why are there negative daily deaths?
counties_vaccination_df %>% filter(state == "Alaska") %>% 
  filter(daily_deaths > -1) %>%
  drop_na(daily_deaths) %>% 
  group_by(county) %>%
  summarize(total_deaths_in_county = sum(daily_deaths))

#verify there are 23 deaths
counties_vaccination_df %>% filter(county == "Bethel Census Area") %>% 
  filter(daily_deaths > -1) %>% 
